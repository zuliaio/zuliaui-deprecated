...
public class QueryView extends Div implements ResizeHandler {
	...
	public void draw(UIQueryResults uiQueryResults) {
		...
		GWT.log("Starting to draw all options");
		QueryOptionsView queryOptionsView = new QueryOptionsView(uiQueryResults);
		leftScrollPanel.setWidget(queryOptionsView);
		GWT.log("Finished drawing all options");
		...
		if (!uiQueryResults.getJsonDocs().isEmpty()) {
			if (!uiQueryResults.getFacetCountsMap().isEmpty()) {
				...
				tabPanel.createAndAddTabPane("facets", new FacetsResultsView(uiQueryResults.getFacetCountsMap()));
